...
class OperationalSpaceWalkingController(LeafSystem):
    ...
    def SetupAndSolveQP(self,  context: Context) -> Tuple[np.ndarray, MathematicalProgram]:
        ...
        for traj_name in self.trajs:
            ...
            # Q_i = 2.0 * J_i.T @ W_i @ J_i
            # b_i_T = 2.0 * (-yddot_cmd_i.T @ W_i @ J_i + JdotV_i.T @ W_i @ J_i)
            # c_i = yddot_cmd_i.T @ W_i @ yddot_cmd_i - 2.0 * yddot_cmd_i.T @ W_i @ JdotV_i + JdotV_i.T @ W_i @ JdotV_i
            # c_i = yddot_cmd_i.T @ W_i @ yddot_cmd_i - yddot_cmd_i.T @ W_i @ JdotV_i - JdotV_i.T @ W_i @ yddot_cmd_i + JdotV_i.T @ W_i @ JdotV_i
            # Q_i = 2.0 * J_i.T @ W_i @ J_i
            # b_i_T = yddot_cmd_i.T @ W_i.T @ J_i - J_i.T @ W_i @ JdotV_i + yddot_cmd_i @ W_i @ J_i - J_i.T @ W_i.T @ JdotV_i
            # c_i = yddot_cmd_i.T @ W_i @ yddot_cmd_i - yddot_cmd_i @ W_i @ JdotV_i - yddot_cmd_i.T @ W_i.T @ JdotV_i + JdotV_i.T @ W_i @ JdotV_i

            Q_i = 2 * J_i.T @ W_i @ J_i
            b_i_T = -yddot_cmd_i.T @ W_i @ J_i - yddot_cmd_i.T @ W_i.T @ J_i + JdotV_i.T @ W_i @ J_i + JdotV_i.T @ W_i.T @ J_i
            c_i = yddot_cmd_i.T @ W_i @ yddot_cmd_i - yddot_cmd_i.T @ W_i @ JdotV_i - JdotV_i.T @ W_i @ yddot_cmd_i + JdotV_i.T @ W_i @ JdotV_i

            prog.AddQuadraticCost(Q_i, b_i_T, c_i, vdot, is_convex=True)
            # prog.AddQuadraticCost(vdot.T @ Q_i @ vdot)
            # prog.AddLinearCost(b_i_T @ vdot)
            # prog.AddCost(c_i)
            # yddot_i = JdotV_i + J_i @ vdot
            # prog.AddQuadraticCost((yddot_i - yddot_cmd_i).T @ W_i @ ( yddot_i))

        # TODO: Add Quadratic Cost on vdot using self.gains.w_vdot * Identity
        Q_eye = self.gains.w_vdot * np.eye(7)
        prog.AddQuadraticCost(2 * Q_eye, np.zeros(7), vdot, is_convex=True)
        ...
        # for i in range(7):
        #     prog.AddLinearEqualityConstraint(vdot[i] == (np.linalg.inv(M) @ (B @ u + J_c.T @ lambda_c - Cv - G))[i])
        prog.AddLinearEqualityConstraint(M @ vdot + Cv + G - B @ u - J_c.T @ lambda_c, np.zeros((7, 1)))
    ...
    def CalcTorques(self, context: Context, output: BasicVector) -> None:
        ...
        # usol = np.array([  25.98501626,  -20.79166107, -160.99593893,  -1.83807018])
        usol = np.array([0, 0, 0, 0])
        lf_pos = self.plant.CalcPointsPositions(
            self.plant_context,
            PointOnFrame(
                self.plant.GetBodyByName("left_lower_leg").body_frame(), np.array([0, 0, -0.5])
            ).frame,
            PointOnFrame(
                self.plant.GetBodyByName("left_lower_leg").body_frame(), np.array([0, 0, -0.5])
            ).pt,
